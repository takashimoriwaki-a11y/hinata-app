...
【最重要：文字の正確な読み取り】
- 画像を拡大して1文字1文字を慎重に読み取ること
- 氏名は漢字を正確に。似た漢字の間違いに注意（例：溝/濱/構、崎/﨑、渡/渡邊、斎/斉/齊）
- ふりがなは画像に記載がある場合のみ。推測で書かない
- 住所の数字・漢字は特に正確に（丁目・番地・号・部屋番号）
- 電話番号は1桁ずつ正確に読む。ハイフン位置も画像通りに
- 手書き文字は筆跡・文脈から最も妥当な文字を判断する
- 読み取れない文字がある場合は「?」を入れるよりも前後の文脈から推測する

【チェックボックス・選択欄の読み取りルール】
- ✓、✔、レ点、○、●、■が付いている項目＝「該当・選択・実施あり」
...
- 生年月日はYYYY-MM-DD形式（例：1940-12-03）。和暦表記の場合は西暦に変換
- 家族構成図がある場合は全員を読み取る（キーパーソンは isKeyPerson: true）
- 受診中の病院・主治医の情報を正確に記載
- 申し送り・特記事項は原文を忠実に転記（重要情報を省略しない）
- 情報がないフィールドは空文字""にする。推測で埋めない
${supplementText ? '\n【補足情報】\n' + supplementText + '\n' : ''}
改行なしの1行コンパクトJSONで返してください:
{"userName":"","nameKana":"","gender":"","age":"","birthDate":"YYYY-MM-DD","address":"","phone":"","mobilePhone":"","mainDiagnosis":"","medicalHistory":"","currentCondition":"","diseaseAwareness":"","familyMembers":[{"name":"","age":"","relationship":"","cohabitation":"","occupation":"","contact":"","isKeyPerson":false}],"familySpecialNotes":"","patientRequest":"","familyRequest":"","referralRequest":"","otherServices":"","handoverNotes":"","bedrideLevel":"","dementiaLevel":"","relatedInstitutions":[{"name":"","phone":"","contact":""}],"referralSource":"","referralPhone":"","familyContact":"","familyPhone":"","familyMobilePhone":"","doctorHospital":"","doctorName":"","supporters":[{"role":"","name":"","phone":"","note":""}]}`;
