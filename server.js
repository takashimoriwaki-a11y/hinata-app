...
app.post('/api/ocr-facesheet', verifySignature, limitOCR, async (req, res) => {
  try {
    ...
    const prompt = `あなたは精神科・在宅医療に精通した訪問看護ステーションの熟練事務スタッフ兼医療記録の専門家です。

【タスク】以下の2つの情報源を照合して、最も正確なデータを抽出してください：
1. 添付画像（原本） — 画像を直接見て文字を1文字ずつ確認
2. 事前OCRテキスト（参考） — 機械読取結果。誤読の可能性あり

画像とOCRテキストで食い違う場合は、画像を優先してください。

【文字読み取りの精度ルール（最重要）】
- 氏名：漢字を1文字ずつ画像で確認。似た字に注意（溝/構、崎/﨑、渡/渡邊、斎/斉/齊、藤/膝）
- ふりがな：画像に明記されている場合のみ記載。推測禁止
- 住所：〒番号→都道府県→市区町村→番地→建物名・号室を画像から正確に転記
- 電話番号：画像の数字を1桁ずつ確認。市外局番・ハイフン位置も正確に
- 生年月日：和暦の場合は西暦に変換（昭和→1925+年、平成→1988+年、令和→2018+年）
- 手書き文字：筆跡と前後の文脈から最も妥当な文字を判断
...
【医療用語変換】
Alz.→アルツハイマー型認知症, DLB→レビー小体型認知症, HTN→高血圧症, DM→2型糖尿病, CKD→慢性腎臓病, COPD→慢性閉塞性肺疾患, CVA→脳梗塞後遺症, BPD→双極性障害, GH→グループホーム, DS→デイサービス, DC→デイケア, CM→ケアマネジャー, PSW→精神科ソーシャルワーカー

【その他】
- 複数画像の場合は全画像の情報を統合
- 家族構成図がある場合は全員を読み取る（キーパーソンはisKeyPerson:true）
- 申し送り・特記事項は原文を忠実に転記
