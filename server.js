...
app.post('/api/delete-data', verifySignature, async (req, res) => {
  try {
    ...
    const searchUrl = `https://www.googleapis.com/drive/v3/files?q=name='patient_${patientId}.json' and '${dataFolderId}' in parents and trashed=false&supportsAllDrives=true&includeItemsFromAllDrives=true`;
    ...
    if (searchData.files && searchData.files.length > 0) {
      for (const file of searchData.files) {
        console.log('[DELETE] deleting file:', file.id);
