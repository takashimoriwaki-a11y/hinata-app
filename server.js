...
// 3. リクエストサイズ制限
app.use(express.json({ limit: '50mb' }));  // PNG画像アップロード用に50MB
...
フェイスシートの画像を読み取り、訪問看護記録書Ⅰの各項目に正確に振り分けてください。

【情報源】画像（原本）とOCRテキスト（参考）の2つ。食い違う場合は画像優先。
【文字精度】氏名の漢字は1文字ずつ確認。ふりがなは記載時のみ。住所・電話番号は正確に。和暦は西暦変換（昭和+1925/平成+1988/令和+2018）。手書きは筆跡と文脈で判断。
【チェックボックス】✓/○/●/■→該当、□/空欄→非該当
