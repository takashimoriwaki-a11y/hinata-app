...
.card { background: white; border-radius: 24px; padding: 32px 28px; max-width: 400px; width: 100%; text-align: center; box-shadow: 0 4px 24px rgba(0,0,0,0.12); }
h1 { font-size: 16px; color: #92400e; font-weight: bold; margin-bottom: 4px; }
p.sub { font-size: 12px; color: #78716c; margin-bottom: 24px; }
#qr-container { display: flex; justify-content: center; margin-bottom: 20px; }
canvas { border: 3px solid #d97706; border-radius: 8px; }
.url-box { background: #fef3c7; border: 1px solid #f59e0b; border-radius: 12px; padding: 10px 14px; margin-bottom: 16px; word-break: break-all; font-size: 12px; color: #1d4ed8; }
.note { font-size: 11px; color: #78716c; line-height: 1.5; }
.badge { background: #92400e; color: white; font-size: 11px; font-weight: bold; padding: 4px 10px; border-radius: 20px; display: inline-block; margin-bottom: 12px; }
</style>
</head>
<body>
<div class="card">
  ...
  <div id="qr-container"><canvas id="qr" width="280" height="280"></canvas></div>
  <div class="url-box">${appUrl}</div>
  <p class="note">
    📱 iPhoneはカメラで読み取るか、上のURLをタップ<br>
    💻 PCはURLをブラウザにコピー<br>
    🔑 @kokoronohinata.com アカウントのみ
  </p>
</div>
<script>
// QRコード生成（qrcodegen by Nayuki, MIT License, inline implementation）
(function(){
  const url = '${appUrl}';
  
  // Canvas に直接描画するシンプルなQR生成
  // URLをData URIとして扱い、Google QR APIの代替としてサーバーサイドで生成したSVGを埋め込む
  const canvas = document.getElementById('qr');
  const ctx = canvas.getContext('2d');
  
  // SVGデータを取得してCanvasに描画
  fetch('/qr-svg')
    .then(r => r.text())
    .then(svg => {
...
app.get('/qr-svg', (req, res) => {
  ...
  let v = 1;
  for (let i = 0; i < 255; i++) { EXP[i]=v; LOG[v]=i; v=(v<<1)^(v>>7?0x11D:0); }
  for (let i = 255; i < 512; i++) EXP[i]=EXP[i-255];
  const mul=(a,b)=>(!a||!b)?0:EXP[(LOG[a]+LOG[b])%255];
  const rsGen=deg=>{let g=[1];for(let i=0;i<deg;i++){const ng=new Array(g.length+1).fill(0);g.forEach((c,j)=>{ng[j]^=c;ng[j+1]^=mul(c,EXP[i]);});g=ng;}return g;};
  const rsEcc=(data,deg)=>{const g=rsGen(deg),r=[...data,...new Array(deg).fill(0)];for(let i=0;i<data.length;i++){const f=r[i];if(f)for(let j=0;j<deg;j++)r[i+1+j]^=mul(f,g[j+1]);}return r.slice(data.length);};
  const raw=Buffer.from(url,'latin1'),n=raw.length;
  const bits=[];[0,1,0,0].forEach(b=>bits.push(b));
  for(let i=7;i>=0;i--)bits.push((n>>i)&1);
  raw.forEach(byte=>{for(let i=7;i>=0;i--)bits.push((byte>>i)&1);});
  bits.push(0,0,0,0);while(bits.length%8)bits.push(0);
  const cw=[];for(let i=0;i<bits.length;i+=8)cw.push(bits.slice(i,i+8).reduce((a,b,j)=>a+(b<<(7-j)),0));
  const pads=[0xEC,0x11];let pi=0;while(cw.length<61)cw.push(pads[pi++%2]);
  const ecc=rsEcc(cw,19),final=[...cw,...ecc];
  const mat=Array.from({length:N},()=>new Array(N).fill(0));
  const fn=Array.from({length:N},()=>new Array(N).fill(false));
  const set=(r,c,vv)=>{mat[r][c]=vv?1:0;fn[r][c]=true;};
  const finder=(tr,tc)=>{
  ...
  for(let i=0;i<8;i++){set(7,i,0);set(i,7,0);set(7,N-1-i,0);set(i,N-8,0);set(N-8,i,0);set(N-1-i,7,0);}
  ...
  const dbits=[];final.forEach(cw2=>{for(let i=7;i>=0;i--)dbits.push((cw2>>i)&1);});
  let bi=0,col=N-1;
  while(col>=1){
    if(col===6)col--;
    const upward=Math.floor((N-1-col)/2)%2===0;
    const rows=upward?Array.from({length:N},(_,i)=>N-1-i):Array.from({length:N},(_,i)=>i);
    rows.forEach(row=>[0,1].forEach(dc=>{const c=col-dc;if(c>=0&&!fn[row][c]){mat[row][c]=bi<dbits.length?dbits[bi++]:0;}}));
    col-=2;
  }
  for(let r=0;r<N;r++)for(let c=0;c<N;c++)if(!fn[r][c]&&(r+c)%2===0)mat[r][c]^=1;
  const fmtD=(0b01<<3)|0;let rem=fmtD;for(let _=0;_<10;_++)rem=(rem<<1)^(rem>>9?0x537:0);
  const fw=((fmtD<<10)|rem)^0x5412;
  const fb=Array.from({length:15},(_,i)=>(fw>>(14-i))&1);
  [[8,0],[8,1],[8,2],[8,3],[8,4],[8,5],[8,7],[8,8],[7,8],[5,8],[4,8],[3,8],[2,8],[1,8],[0,8]].forEach(([r,c],i)=>mat[r][c]=fb[i]);
  ...
  const svg=`<?xml version="1.0"?><svg xmlns="http://www.w3.org/2000/svg" width="${SZ}" height="${SZ}" viewBox="0 0 ${SZ} ${SZ}" shape-rendering="crispEdges"><rect width="${SZ}" height="${SZ}" fill="white"/><g fill="black">${rects.join('')}</g></svg>`;
