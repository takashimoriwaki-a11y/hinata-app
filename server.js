...
// 3. リクエストサイズ制限
app.use(express.json({ limit: '100mb' }));  // PNG画像複数枚アップロード用に100MB
...
app.get('/api/sheets-mikomi', async (req, res) => {
  try {
    const mikomiSheetsId = '1cJ8f3gFWu0Fqrl3TxthGVk0-9TF4Hg5YJZFO-mWIvjI';
    const serviceAccount = JSON.parse(process.env.GOOGLE_SERVICE_ACCOUNT_JSON);
    const token = await getAccessTokenForSheets(serviceAccount);
    const range = encodeURIComponent('見込み件数!B46:G53');
    const url = `https://sheets.googleapis.com/v4/spreadsheets/${mikomiSheetsId}/values/${range}`;
    const r = await fetch(url, { headers: { Authorization: `Bearer ${token}` } });
    const data = await r.json();
    if (data.error) return res.json({ success: false, error: data.error.message || 'シート取得失敗' });
    res.json({ success: true, values: data.values || [] });
  } catch (err) {
    console.error('sheets-mikomi error:', err);
...
app.post('/api/ocr-prescription', verifySignature, limitOCR, async (req, res) => {
  try {
    ...
    // Vision OCRでテキスト抽出（Geminiの補助情報）
    let ocrText = '';
    try {
      const token = await getAccessTokenForVision(serviceAccount);
      ocrText = await visionOCR(token, base64Image, mimeType);
    } catch(e) { console.warn('処方OCR Vision失敗（続行）:', e.message); }

    // Geminiで処方内容を構造化抽出
    const parts = [
      { inline_data: { mime_type: mimeType, data: base64Image } },
      { text: `この処方箋・お薬手帳・薬情の画像から、処方内容を正確に読み取ってください。

【最重要ルール】
- 薬剤名（商品名）は必ず記載する。薬剤名なしで用法だけ書くのは禁止
- 規格（mg, μg, mL等）も正確に読み取る
- 用法（1日○回、毎食後 等）と用量（○錠、○包 等）を薬剤ごとに記載

【出力フォーマット】
薬剤名 規格 用法 用量 の順に、1行1薬剤で記載。例：
・リスペリドン錠1mg 1日2回 朝夕食後 1回1錠
・酸化マグネシウム錠330mg 1日3回 毎食後 1回1錠
・フルニトラゼパム錠1mg 1日1回 就寝前 1回1錠

画像に処方日・医療機関・薬局の情報があれば最後に記載。
${ocrText ? '\n【参考OCRテキスト】\n' + ocrText.slice(0, 3000) : ''}` }
    ...
    try {
      ...
      if (candidate?.content?.parts?.[0]?.text) {
        ...
        console.log('処方OCR Gemini成功:', text.slice(0, 100));
        return res.json({ success: true, text: text });
      }
    } catch(e) { console.warn('処方OCR Gemini失敗、フォールバック:', e.message); }

    // フォールバック: Vision OCRテキストをそのまま返す
    const result = parsePrescription(ocrText);
...
JSONのみ返すこと。` }
      ...
      const geminiData = await geminiRes.json();
      if (!geminiRes.ok) {
        console.error('Gemini名刺APIエラー:', JSON.stringify(geminiData).slice(0, 500));
      }
      const candidate = (geminiData.candidates || [])[0];
      if (candidate?.content?.parts?.[0]?.text) {
    ...
    } catch(e) { console.error('Gemini名刺解析エラー:', e.message); }

    // フォールバック：Vision OCR + ルールベース解析
    const token = await getAccessTokenForVision(serviceAccount);
...
【各フィールドの意味と振り分けルール】
●userName: 利用者の氏名（フルネーム）
●nameKana: ふりがな（ひらがな。画像に記載がある場合のみ）
●gender: 性別（"男"or"女"）
●age: 年齢（数字のみ）
●birthDate: 生年月日（YYYY-MM-DD形式）
●address: 住所（〒郵便番号から建物名・号室まで完全に）
●phone: 固定電話（ハイフン付き）
●mobilePhone: 携帯電話（ハイフン付き）
●mainDiagnosis: 主傷病名（メインの診断名1つ。正式な医学用語で）
●medicalHistory: 【既往歴及び現病歴】過去にかかった病気・手術歴・発症時期・経過。現在の病気がいつ発症しどう経過したか。入院歴。（360字以内）
●currentCondition: 【病状・治療状態・処方内容】現在の症状・状態、通院中の病院と頻度、受けている治療、服用中の薬の名前と量。（440字以内）
●diseaseAwareness: 【病識の有無/程度】本人が自分の病気をどの程度理解・自覚しているか。「病識あり」「一部理解」「病識なし」等。（280字以内）
●familyMembers: 家族構成（配列）。name/age/relationship(続柄)/cohabitation(同居or別居)/occupation/contact(電話)/isKeyPerson(キーパーソンならtrue)
●familySpecialNotes: 【家族構成特記事項】家族関係の問題・キーパーソン不在・独居の背景等。（280字以内）
●patientRequest: 【本人の要望】利用者本人が訪問看護に求めること・希望・目標。（273字以内）
●familyRequest: 【家族の要望】家族が訪問看護に求めること・希望・心配事。（273字以内）
●referralRequest: 【依頼機関の要望】依頼元（病院・ケアマネ等）が訪問看護に求めること・依頼理由。（273字以内）
●otherServices: 【その他サービスの利用状況】デイサービス・ヘルパー・デイケア・訪問リハ・ショートステイ等、訪問看護以外に利用中のサービス一覧。（320字以内）
●handoverNotes: 【スタッフへの申し送り事項】上記のどの項目にも当てはまらない情報は全てここに入れる。注意事項・生活背景・経済状況・住環境・近隣との関係・趣味嗜好・宗教・アレルギー・禁忌事項・過去のトラブル・コミュニケーション上の注意点等。フェイスシートの情報で他の項目に振り分けられなかったものは全てここ。（800字以内）
●bedrideLevel: 障害老人日常生活自立度（J1/J2/A1/A2/B1/B2/C1/C2のいずれか。記載なしは""）
●dementiaLevel: 認知症日常生活自立度（Ⅰ/Ⅱa/Ⅱb/Ⅲa/Ⅲb/Ⅳ/Mのいずれか。記載なしは""）
●relatedInstitutions: 関連機関（配列）。かかりつけ病院・薬局・相談支援事業所等。name/phone/contact(担当者名)
●referralSource: 依頼元機関名（紹介元の病院・相談支援事業所・ケアマネ事業所等）
●referralPhone: 依頼元の電話番号
●familyContact: 緊急連絡先の氏名（家族等）
●familyPhone: 緊急連絡先の固定電話
●familyMobilePhone: 緊急連絡先の携帯電話
●doctorHospital: 主治医の所属病院名
●doctorName: 主治医の氏名
●supporters: 支援者（配列）。ケアマネ・相談員・PSW等。role/name/phone/note(所属)

【ADL（日常生活動作）の振り分け — 記載がある場合のみ】
各項目のsupport("要援助"or"援助不要"or"")とlevel("自立"or"一部介助"or"全介助"or"")を判定：
●adlEating: 食生活（食事の準備・摂取・偏食・栄養状態）
●adlHygiene: 清潔（入浴・洗面・歯磨き・着替え・身だしなみ）
●adlToileting: 排泄（トイレ動作・失禁の有無・オムツ使用）
●adlSleep: 睡眠（睡眠パターン・不眠・昼夜逆転）
●adlDailyRhythm: 生活のリズム（日中の過ごし方・活動と休息のバランス・引きこもり）
●adlMedication: 服薬状況（服薬管理・飲み忘れ・拒薬・自己調整）
●adlFinance: 金銭管理（家計・買い物・浪費・金銭トラブル）
●adlDayCare: デイケア・作業所（通所状況・参加意欲・社会参加）
●adlInterpersonal: 対人関係（他者との関わり・孤立・トラブル・コミュニケーション能力）
●adlNurseComm: 看護師とのコミュニケーション（医療者との信頼関係・相談できるか）
判定目安：「自立/できる/問題なし」→援助不要+自立、「見守り/声かけ/促し」→要援助+自立、「一部介助/手伝い」→要援助+一部介助、「全介助/できない」→要援助+全介助

【日常生活自立度の判定（記載がある場合のみ）】
●bedrideLevel: 障害老人日常生活自立度。以下から最も近いものを1つ選択（記載なしは""）
 J1=交通機関を利用して外出する, J2=隣近所へなら外出する
 A1=介助で外出し日中はベッドから離れて生活, A2=外出頻度少なく日中も寝たり起きたり
 B1=車いすに移乗し食事排泄はベッドから離れる, B2=介助で車いすに移乗
 C1=自力で寝返りをうつが排泄食事着替えに介助要, C2=自力で寝返りもうてない
●dementiaLevel: 認知症日常生活自立度。以下から最も近いものを1つ選択（記載なしは""）
 Ⅰ=日常生活はほぼ自立, Ⅱa=家庭外で支障あり, Ⅱb=家庭内でも支障あり
 Ⅲa=日中中心に介護必要, Ⅲb=夜間中心に介護必要
 Ⅳ=常に介護を必要とする, M=著しい精神症状や重篤な身体疾患で専門医療必要

【重要：情報の振り分けルール】
- フェイスシートと補足テキストの全情報を漏れなく各項目に正確かつ詳細に振り分ける
- 1つの情報は最も適切な1つの項目に入れる（重複記載しない）
- handoverNotes: FSに「申し送り」「特記事項」「注意事項」として記載されている内容のみ
- otherInfo: 上記の全フィールドに振り分けられなかった残り全ての情報をここに集約する（生活背景・経済状況・住環境・趣味嗜好・アレルギー・宗教・その他の注釈等）
- 情報がないフィールドは空文字""。推測で埋めない
${supplementText ? '\n【補足テキスト（利用者の追加情報。各項目に正確に振り分けること）】\n' + supplementText + '\n' : ''}
${visionOcrResult ? '【事前OCRテキスト（参考・誤読注意）】\n' + visionOcrResult.slice(0, 6000) + '\n\n' : ''}改行なしの1行コンパクトJSONで返してください:
{"userName":"","nameKana":"","gender":"","age":"","birthDate":"","address":"","phone":"","mobilePhone":"","mainDiagnosis":"","medicalHistory":"","currentCondition":"","diseaseAwareness":"","familyMembers":[],"familySpecialNotes":"","patientRequest":"","familyRequest":"","referralRequest":"","otherServices":"","handoverNotes":"","bedrideLevel":"","dementiaLevel":"","relatedInstitutions":[],"referralSource":"","referralPhone":"","familyContact":"","familyPhone":"","familyMobilePhone":"","doctorHospital":"","doctorName":"","supporters":[],"adlEating":{"support":"","level":""},"adlHygiene":{"support":"","level":""},"adlToileting":{"support":"","level":""},"adlSleep":{"support":"","level":""},"adlDailyRhythm":{"support":"","level":""},"adlMedication":{"support":"","level":""},"adlFinance":{"support":"","level":""},"adlDayCare":{"support":"","level":""},"adlInterpersonal":{"support":"","level":""},"adlNurseComm":{"support":"","level":""},"otherInfo":""}`;
    ...
    try {
      ...
      const geminiData = await geminiRes.json();
      console.log('Gemini FSレスポンス status=' + geminiRes.status + ', candidates=' + (geminiData.candidates || []).length);
      if (!geminiRes.ok) {
        console.error('Gemini FSスキャンAPIエラー:', JSON.stringify(geminiData).slice(0, 800));
      }
      const candidate = (geminiData.candidates || [])[0];
      if (candidate?.content?.parts?.[0]?.text) {
        const rawContent = candidate.content.parts[0].text.trim();
        console.log('Gemini FS生テキスト（先頭200文字）:', rawContent.slice(0, 200));
        // ```json ... ``` を除去してJSONだけ取り出す
        ...
        // JSON内の制御文字・改行を全て除去して1行に圧縮
        jsonStr = jsonStr.replace(/[\x00-\x1f\x7f]+/g, ' ').replace(/\s+/g, ' ');
        // nullを空文字に
        jsonStr = jsonStr.replace(/:\s*null\b/g, ': ""');
        try {
          const parsed = JSON.parse(jsonStr);
          console.log('Gemini FSパース成功: userName=' + (parsed.userName || '空') + ', fields=' + Object.keys(parsed).filter(k => parsed[k] && parsed[k] !== '').length);
          parsed.extractedAt = new Date().toISOString();
          return res.json({ success: true, data: parsed });
        } catch (jsonErr) {
          console.error('JSONパースエラー:', jsonErr.message);
          console.error('JSON文字列（先頭300文字）:', jsonStr.slice(0, 300));
          // 最終手段: 各フィールドを正規表現で個別抽出
          const extract = (key) => { const m = jsonStr.match(new RegExp('"' + key + '"\\s*:\\s*"([^"]*)"')); return m ? m[1] : ''; };
          const extractArr = (key) => { const m = jsonStr.match(new RegExp('"' + key + '"\\s*:\\s*(\\[.*?\\])', 's')); try { return m ? JSON.parse(m[1]) : []; } catch(e) { return []; } };
          const fallbackParsed = {
            userName: extract('userName'), nameKana: extract('nameKana'), gender: extract('gender'),
            age: extract('age'), birthDate: extract('birthDate'), address: extract('address'),
            phone: extract('phone'), mobilePhone: extract('mobilePhone'),
            mainDiagnosis: extract('mainDiagnosis'), medicalHistory: extract('medicalHistory'),
            currentCondition: extract('currentCondition'), diseaseAwareness: extract('diseaseAwareness'),
            patientRequest: extract('patientRequest'), familyRequest: extract('familyRequest'),
            referralRequest: extract('referralRequest'), otherServices: extract('otherServices'),
            handoverNotes: extract('handoverNotes'), bedrideLevel: extract('bedrideLevel'),
            dementiaLevel: extract('dementiaLevel'), familySpecialNotes: extract('familySpecialNotes'),
            referralSource: extract('referralSource'), referralPhone: extract('referralPhone'),
            familyContact: extract('familyContact'), familyPhone: extract('familyPhone'),
            familyMobilePhone: extract('familyMobilePhone'),
            doctorHospital: extract('doctorHospital'), doctorName: extract('doctorName'),
            familyMembers: extractArr('familyMembers'), supporters: extractArr('supporters'),
            relatedInstitutions: extractArr('relatedInstitutions'),
            extractedAt: new Date().toISOString(),
          };
          console.log('正規表現抽出フォールバック: userName=' + (fallbackParsed.userName || '空'));
          return res.json({ success: true, data: fallbackParsed });
        }
      } else {
        console.error('Gemini FS: candidateにテキストなし', JSON.stringify(geminiData).slice(0, 500));
      }
    } catch (aiErr) {
      console.warn('Gemini解析失敗、正規表現パーサーにフォールバック:', aiErr.message);
    }

    console.log('フォールバック: 正規表現パーサー使用');

    // フォールバック: 既にVision OCRで取得済みのテキストを使用
    let allText = visionOcrResult;
    if (supplementText) allText += '\n' + sanitizeString(supplementText, 5000);
...
async function getAccessTokenForGemini(serviceAccount) {
  return _getToken(serviceAccount, 'https://www.googleapis.com/auth/cloud-platform https://www.googleapis.com/auth/generative-language');
}
