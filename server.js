...
app.post('/api/ocr-card', verifySignature, limitOCR, async (req, res) => {
  try {
    ...
    // Step2: Gemini AIで名前・役職・電話番号を正確に解析
    if (rawText.trim()) {
      try {
        const geminiToken = await getAccessTokenForVision(serviceAccount);
        const projectId = serviceAccount.project_id;
        const geminiRes = await fetch(`https://us-central1-aiplatform.googleapis.com/v1/projects/${projectId}/locations/us-central1/publishers/google/models/gemini-2.0-flash:generateContent`, {
          method: 'POST',
          headers: { Authorization: `Bearer ${geminiToken}`, 'Content-Type': 'application/json' },
          body: JSON.stringify({
            contents: [{ role: 'user', parts: [{ text: `あなたは名刺情報を解析する専門家です。OCRで読み取った名刺テキストから以下のJSONを抽出してください。
