...
function buildRows(c, e) {
  ...
  let rowIdx = 0;
  const SEC = (label) => { rows.push([{ v: label, bold: true, bg: '#2e7d9a', color: '#ffffff', merge: true }]); rowIdx++; };
...
async function sheetsFormatAndValidate(token, spreadsheetId, sheetId, rows, validations) {
  ...
  rows.forEach((row, rowIndex) => {
    ...
    if (cell0.bold && cell0.bg) {
      ...
      requests.push({ repeatCell: { range: { sheetId, startRowIndex: rowIndex, endRowIndex: rowIndex+1, startColumnIndex: 0, endColumnIndex: 2 },
        cell: { userEnteredFormat: { backgroundColor: { red:0.78, green:0.90, blue:0.95 }, textFormat: { bold: true, fontSize: 10, foregroundColor: { red:0, green:0, blue:0 } }, horizontalAlignment: 'LEFT' } },
        fields: 'userEnteredFormat(backgroundColor,textFormat,horizontalAlignment)' } });
      requests.push({ mergeCells: { range: { sheetId, startRowIndex: rowIndex, endRowIndex: rowIndex+1, startColumnIndex: 0, endColumnIndex: 2 }, mergeType: 'MERGE_ALL' } });
    } else if (cell0.ibow) {
      // ibow登録電話番号は水色でハイライト
  ...
  try {
    ...
    if (searchData.files && searchData.files.length > 0) {
      for (const file of searchData.files) {
        console.log('[DELETE] deleting file:', file.id, 'driveId:', file.driveId);
        // 共有ドライブのファイルはdriveIdを指定して削除
        const driveParam = file.driveId ? `&driveId=${file.driveId}` : '';
        const delUrl = `https://www.googleapis.com/drive/v3/files/${file.id}?supportsAllDrives=true&includeItemsFromAllDrives=true${driveParam}`;
        const delRes = await fetch(delUrl, {
          method: 'DELETE',
          headers: { Authorization: `Bearer ${token}` },
        });
        console.log('[DELETE] delete status:', delRes.status);
        if (delRes.status === 403 || delRes.status === 404) {
          // 直接削除できない場合はDrive API v3のfiles.deleteを別パラメータで試みる
          const delRes2 = await fetch(
            `https://www.googleapis.com/drive/v3/files/${file.id}?supportsAllDrives=true`,
            { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } }
          );
          console.log('[DELETE] retry status:', delRes2.status);
        }
      }
    } else {
      console.log('[DELETE] no files found for patientId:', patientId, 'in folder:', dataFolderId);
    }
