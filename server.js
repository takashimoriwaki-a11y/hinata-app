...
app.post('/api/ocr-facesheet', verifySignature, limitOCR, async (req, res) => {
  try {
    ...
    const prompt = `あなたは精神科・在宅医療に精通した訪問看護ステーションの熟練事務スタッフ兼医療記録の専門家です。訪問看護フェイスシート・情報提供書・アセスメント票・ケアプランの画像を精密に読み取ります。
...
【医療用語の正確な変換】
- Alz. → アルツハイマー型認知症, DLB → レビー小体型認知症, HTN → 高血圧症, DM → 2型糖尿病
- CKD → 慢性腎臓病, COPD → 慢性閉塞性肺疾患, CVA → 脳梗塞後遺症, 統合失調症 → 統合失調症
- BPD → 双極性障害, GH → グループホーム, DS → デイサービス, DC → デイケア
- CM → ケアマネジャー, PSW → 精神科ソーシャルワーカー, OT → 作業療法士, PT → 理学療法士
...
{"userName":"利用者氏名","nameKana":"ふりがな（ひらがな）","gender":"男 or 女","age":"年齢（数字のみ）","birthDate":"YYYY-MM-DD","address":"〒含む完全な住所","phone":"固定電話（ハイフン付き）","mobilePhone":"携帯電話（ハイフン付き）","mainDiagnosis":"主傷病名","medicalHistory":"既往歴・現病歴（360字以内）","currentCondition":"現在の病状・治療内容・処方薬（440字以内）","diseaseAwareness":"病識の有無・程度（280字以内）","familyMembers":[{"name":"氏名","age":"年齢","relationship":"続柄","cohabitation":"同居 or 別居","occupation":"職業","contact":"電話番号","isKeyPerson":false}],"familySpecialNotes":"家族構成の特記事項（280字以内）","patientRequest":"本人の要望（273字以内）","familyRequest":"家族の要望（273字以内）","referralRequest":"依頼機関からの要望（273字以内）","otherServices":"利用中の他サービス（320字以内）","handoverNotes":"申し送り・特記事項（800字以内）","bedrideLevel":"J1/J2/A1/A2/B1/B2/C1/C2 or 空","dementiaLevel":"Ⅰ/Ⅱa/Ⅱb/Ⅲa/Ⅲb/Ⅳ/M or 空","relatedInstitutions":[{"name":"機関名","phone":"電話番号","contact":"担当者名"}],"referralSource":"依頼元機関名","referralPhone":"依頼元電話番号","familyContact":"緊急連絡先氏名","familyPhone":"緊急連絡先固定電話","familyMobilePhone":"緊急連絡先携帯電話","doctorHospital":"主治医の病院名","doctorName":"主治医名","supporters":[{"role":"役割","name":"氏名","phone":"電話番号","note":"所属・備考"}]}`;
    ...
    try {
      ...
      if (candidate?.content?.parts?.[0]?.text) {
        ...
        // JSON文字列値内の改行をエスケープ（"..." 内の生\nを\\nに）
        jsonStr = jsonStr.replace(/"((?:[^"\\]|\\.)*)"/g, (match) => {
          return match.replace(/\n/g, '\\n').replace(/\r/g, '\\r').replace(/\t/g, '\\t');
        });
        // nullをダブルクォート空文字に（null値は空文字として扱う）
    ...
    // フォールバック: Vision OCR + 正規表現パーサー
    let allText = '';
    const visionToken = await getAccessTokenForVision(serviceAccount);
    for (const img of (images || [])) {
      const b64 = sanitizeBase64(img.base64);
      if (!b64) continue;
      const mt = ['image/jpeg', 'image/png', 'image/heic', 'image/webp'].includes(img.mimeType) ? img.mimeType : 'image/jpeg';
      const t = await visionOCR(visionToken, b64, mt);
      allText += t + '\n\n';
    }
