      ...
      try {
        const claudeRes = await fetch('https://api.anthropic.com/v1/messages', {
          ...
          body: JSON.stringify({
            ...
            max_tokens: 2000,
            system: `あなたは訪問看護ステーションの事務スタッフです。OCRで読み取ったフェイスシート・情報提供書のテキストから情報を抽出し、必ずJSONのみで返してください。前置きや説明は不要です。
抽出するフィールド:
- userName: 利用者氏名（漢字）
- nameKana: ふりがな
- gender: 性別（男/女）
...
- phone: 電話番号（固定）
- mobilePhone: 携帯電話
- mainDiagnosis: 主病名・診断名
- medicalHistory: 既往歴
- currentCondition: 現在の病状・治療内容
- diseaseAwareness: 病気の認識（本人・家族）
- patientRequest: 本人の要望・希望
- familyRequest: 家族の要望・希望
- referralRequest: 依頼内容
- familyMembers: 家族構成（配列: [{name, relation, isKeyperson}]）
- familySpecialNotes: 家族に関する特記事項
- otherServices: 他サービス利用状況
- handoverNotes: 申し送り・特記事項
- bedrideLevel: 障害老人日常生活自立度（J1/J2/A1/A2/B1/B2/C1/C2）
- dementiaLevel: 認知症日常生活自立度（Ⅰ/Ⅱa/Ⅱb/Ⅲa/Ⅲb/Ⅳ/M）
- relatedInstitutions: 関連機関（配列: [{name, role, phone}]）
- referralSource: 依頼元・紹介元
- referralPhone: 依頼元電話番号
- familyContact: 家族連絡先（氏名・続柄）
- familyPhone: 家族電話番号
- doctorHospital: 主治医病院名
- doctorName: 主治医名
- supporters: 支援者（配列: [{role, name, phone, note}]）
情報がない場合は空文字または空配列にしてください。JSONのみ返してください。`,
...
async function uploadFile(token, folderId, fileName, base64Data, mimeType) {
  ...
  const searchRes = await fetch(
    `https://www.googleapis.com/drive/v3/files?q=name='${safeName}' and '${folderId}' in parents and trashed=false&supportsAllDrives=true&includeItemsFromAllDrives=true`,
    { headers: { Authorization: `Bearer ${token}` } }
  );
  const searchData = await searchRes.json();
  const existingFiles = searchData.files || [];

  // 重複ファイルを全て削除（上書きではなく削除→新規作成で確実に1件にする）
...
async function sheetsFormatAndValidate(token, spreadsheetId, sheetId, rows, validations) {
  ...
  rows.forEach((row, rowIndex) => {
    ...
    if (cell0.bold && cell0.bg) {
      ...
      requests.push({ repeatCell: { range: { sheetId, startRowIndex: rowIndex, endRowIndex: rowIndex+1, startColumnIndex: 0, endColumnIndex: 2 },
        cell: { userEnteredFormat: { backgroundColor: { red:0.89, green:0.95, blue:0.99 }, textFormat: { bold: true, fontSize: 10 }, horizontalAlignment: 'LEFT' } },
        fields: 'userEnteredFormat(backgroundColor,textFormat,horizontalAlignment)' } });
      requests.push({ mergeCells: { range: { sheetId, startRowIndex: rowIndex, endRowIndex: rowIndex+1, startColumnIndex: 0, endColumnIndex: 2 }, mergeType: 'MERGE_ALL' } });
    } else if (cell0.ibow) {
      // ibow登録電話番号は水色でハイライト
      requests.push({ repeatCell: { range: { sheetId, startRowIndex: rowIndex, endRowIndex: rowIndex+1, startColumnIndex: 0, endColumnIndex: 2 },
        cell: { userEnteredFormat: { backgroundColor: { red:0.68, green:0.85, blue:0.90 }, textFormat: { bold: true, fontSize: 10 }, horizontalAlignment: 'LEFT' } },
        fields: 'userEnteredFormat(backgroundColor,textFormat,horizontalAlignment)' } });
    } else if (cell0.label) {
...
.qr-wrap{display:flex;justify-content:center;margin-bottom:18px;padding:14px;border:3px solid #d97706;border-radius:12px;background:white;}
...
<div class="card">
  ...
  <p class="note">
    ...
    // qrcodeパッケージがない場合はURLのみ表示
    res.setHeader('Content-Type', 'text/html; charset=utf-8');
    res.send(`<!DOCTYPE html><html><body style="background:#ffe0b2;font-family:sans-serif;padding:40px;text-align:center;">
      <h2 style="color:#92400e;">契約・看護記録Ⅰアプリ</h2>
      <p style="margin:20px 0;">アプリURL：</p>
      <a href="https://takashimoriwaki-a11y.github.io/hinata-app/" 
         style="color:#1d4ed8;font-size:14px;word-break:break-all;">
        https://takashimoriwaki-a11y.github.io/hinata-app/
      </a>
    </body></html>`);
  }
});
...
const DATA_FOLDER_NAME = 'hinata-app-data';
...
// 利用者削除
app.delete('/api/delete-data/:patientId', verifySignature, async (req, res) => {
  try {
    const patientId = req.params.patientId.replace(/[^a-zA-Z0-9_-]/g, '');
    if (!patientId) return safeError(res, 400, '不正なIDです');
    ...
    if (searchData.files && searchData.files.length > 0) {
      await fetch(
        `https://www.googleapis.com/drive/v3/files/${searchData.files[0].id}?supportsAllDrives=true`,
        { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } }
      );
    }
