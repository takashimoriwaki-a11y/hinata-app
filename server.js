    ...
    if (rawText.trim()) {
      try {
        ...
        const geminiRes = await fetch('https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent', {
          ...
          body: JSON.stringify({
            contents: [{ parts: [{ text: `あなたは名刺情報を解析する専門家です。OCRで読み取った名刺テキストから以下のJSONを抽出してください。
...
${rawText.slice(0, 1000)}` }] }],
        ...
        if (geminiRes.ok) {
          ...
          const content = (geminiData.candidates || [])[0]?.content?.parts?.map(p => p.text || '').join('') || '';
      ...
      } catch(e) { console.error('Gemini名刺解析エラー:', e); }
    }
...
${allText.slice(0, 8000)}` }] }],
        ...
        if (candidate?.content?.parts?.[0]?.text) {
          ...
          const raw = rawContent.replace(/^[\s\S]*?({[\s\S]*})[\s\S]*$/, '$1') || rawContent;
